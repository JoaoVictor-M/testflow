...
function StatCard({ title, value, icon }) {
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-gray-200 dark:border-neutral-800 flex items-center space-x-4 transition-colors" title={title}>
      {icon}
      <div>
...
function StatList({ title, data, unit = '', color = 'text-blue-600' }) {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-gray-200 dark:border-neutral-800 h-full transition-colors" title={title}>
      <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">{title}</h2>
      {validData.length > 0 ? (
        <ul className="space-y-3">
          {validData.map((item, index) => (
            <li key={item._id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-neutral-800 rounded-md transition-colors">
...
function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white dark:bg-neutral-900 rounded-lg shadow border border-gray-200 dark:border-neutral-800 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-gray-200 dark:border-neutral-800 transition-colors" title="Distribuição de status de todos os cenários de teste (Passou, Com Erro, Aguardando)">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-gray-200 dark:border-neutral-800 transition-colors" title="Distribuição de status de todas as demandas (Pendente, Testando, Aguardando Correção, Testado)">
        ...
        {filterProjectId === 'all' && horasData.length > 0 && (
          <div className="lg:col-span-3 bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-gray-200 dark:border-neutral-800 transition-colors" title="Horas estimadas totais para cada projeto (visível apenas quando 'Todos os Projetos' está selecionado)">
        ...
        {demandasMaisBugs.length > 0 && (
          <div className="lg:col-span-3 bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-gray-200 dark:border-neutral-800 transition-colors" title="As 5 demandas com a maior contagem de cenários marcados como 'Com Erro'">
            ...
            <ul className="space-y-3">
              {demandasMaisBugs.map((dem, index) => (
                <li key={dem._id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-neutral-800 rounded-md transition-colors">
